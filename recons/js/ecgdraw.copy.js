		...
		if (realIdx != undefined) {
			...
			if (clickY >= (dataobj.y - 30)) {
				...
				if ( cout > 1){
					...
					$('#curselect').text("当前选中" + (cout) + "个心搏，共有 " + (g_bottom_total_idx-1) + " 个心搏")
				}else{
					//$('#curselect').text("当前选中时间： " + g_bottom_min + " 分 " + g_bottom_sec + " 秒；当前选中第" + (g_bottom_idx + 1) + "个心搏，共有 " + g_bottom_total_idx + " 个心搏")
					$('#curselect').text("当前选中第" + (g_bottom_idx + 1) + "个心搏，共有 " + (g_bottom_total_idx-1) + " 个心搏")
				}
//				$('#curselect').text("当前选中时间： " + g_bottom_min + " 分 " + g_bottom_sec + " 秒；当前选中第" + (g_bottom_idx + 1) + "个心搏，共有 " + g_bottom_total_idx + " 个心搏")
			}
		} else {
	...
	function initGsubdatas(realIdx) {
		...
		for (let i = realIdx - 75; i < realIdx; i++) {
			...
			let y = 320 - (obj.value * 160) / (g_pixPmm * g_mmPmV);
			obj.x = x
			obj.y = y
	...
	function gridQRSHeat(qrsctxbuf, realIdx) {
		...
		for (let idx in g_subdatas) {
			...
			let y = 320 - (data.value * 160) / (g_pixPmm * g_mmPmV);
			if (idx == 0) {
	...
	function gridQRSPos(g_subdatas) {
		...
		for (let idx in g_subdatas) {
			...
			if (itemobj.qrs != undefined && itemobj.isdraw != undefined && itemobj.isdraw == 1) {
				...
				let x = (idx * qrscanvas.width) / g_subdatas.length;
				let y = 320 - (itemobj.value * 160) / (g_pixPmm * g_mmPmV);
	...
	function gridQRSLow(qrsgridctxbuf, qrscanvas) {
		...
		let ifulline = 0;
		for (let i = 16; i < (qrswidth); i = i + 16) {
	...
	dealDataRecover = function (data) {
		...
		// 2. 获取原始采样点
		getHeartDataByUrl("http://192.168.9.220:8081/pacs/get_ecg_voltage?file=" + result.rrDats + "&begin=1&end=300&filter=0&reduce_sampling_rate=0", function (ecgdata, startTime, dataLength) {
