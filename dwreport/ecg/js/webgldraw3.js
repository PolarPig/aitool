var ecgdraw = function(id) {
	...
	var _realData = function(that) {
		...
		function cavasToWebGL(width,height, cx, cy) {
		...
		function dealArrayBuff(){
			...
			let canvasx = parseInt( (g_sub_idx * width) / 2250 + off_w*360);
			let canvasy = parseInt(10 - (data * 19) / 100 + idx*20 + off_h * 220);
			let wgxy = cavasToWebGL(1130,2200,canvasx,canvasy)
			
			let x = wgxy.gx
			let y = wgxy.gy
			
//			let x = -1+(g_sub_idx * 0.5) / 2250 + (off_w)*0.6;
...
var drawGrid = function(gridId){
	
	let lowgirdvas = document.getElementById(gridId)
	//低格画笔
	let ctx = lowgirdvas.getContext('2d');
	let width = lowgirdvas.width
	let height = lowgirdvas.height
	// 填充背景颜色
