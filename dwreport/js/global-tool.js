...
//	console.log(result)
	...
	for (let i = 0; i < 100; i++) {
		idxNum1 = idx + i
		if(i == 0 && g_modifAreaIdx == idxNum1){
			flags = true
			break
		}
		if(idxdatasmodif.length > 0 && idxdatasmodif[idxNum1]!=undefined){
			break
		}
		if (g_modifAreaIdx == idxNum1) {
			idx = idxNum1
			flags = true
			break
		}
	}
	for (let i = 0; (i < 100) &&(!flags); i++) {
		idxNum2 = idx - i
		if (g_modifAreaIdx == idxNum2) {
			idx = idxNum2
			flags = true
		}
	}
	
	if ( g_modifAreaIdx != undefined && flags){
		idxitem.modif = 1
		g_modifAreaIdx = undefined;
	}
