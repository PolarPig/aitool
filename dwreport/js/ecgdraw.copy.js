...
var ecgdraw = function () {
	...
	function drawChartNav(navchartctx) {
		...
		for (let i in mdxdatas) {
			...
			let y = 80 - (data.heartRate * 37) / 100;// 心率趋势图不与增益相关联
			if (flag == 0) {
		...
		for (let idx in g_subdatas) {
			...
			if (data.qrs != undefined && g_check_qrsinfo !=undefined  && g_check_qrsinfo.qrsInfo != undefined) {
				if (data.qrs == "P1") {
					...
					g_check_qrsinfo.qrsInfo.qrs_s_w = data.idx;
//					g_check_qrsinfo.qrsInfo.q_w = data.idx
				} else if (data.qrs == "R" && !rwflag) {
					g_check_qrsinfo.qrsInfo.r_w = data.idx
					g_check_qrsinfo.subidx =  idxdatas[g_check_qrsinfo.idx].subidx;
					
					//idxdatas
					let idxdatas_item = idxdatas[g_check_qrsinfo.idx]
					idxdatas[g_check_qrsinfo.idx] = undefined
					
					g_check_qrsinfo.idx = data.idx
					idxdatas_item.idx = data.idx
					idxdatas[data.idx]=idxdatas_item
					
					data.qrsInfo = g_check_qrsinfo.qrsInfo
					rwflag = true;

					//g_clickArea
					g_clickArea.idx = g_check_qrsinfo.idx;
	...
	function drawClickRect() {
		if (g_clickArea != undefined && g_clickArea.length > 0 && g_isSelected == 0) {
			ctxrect.beginPath();
			ctxrect.lineWidth = "1";
			ctxrect.strokeStyle = "red";
			for (let idx in g_clickArea) {
...
//					console.log(idxdatas[idx])
				...
				// 如果有选中的单搏 g_clickArea
				if(g_clickArea!=undefined && g_clickArea != [] && g_clickArea.idx == idx && g_isSelected == 0){
					ctxtext.beginPath();
					ctxtext.lineWidth = "1";
					ctxtext.strokeStyle = "red";
					ctxtext.strokeRect(x -  15, y - 10, 30, 60);
					ctxtext.stroke();
					g_isSelected = 1
				}
	...
	function qrscanvasClick(e) {
		...
		for (let idx in g_subdatas) {
			...
			if(pos.qrs != undefined){
				let temp = Math.abs(clickX - pos.x)
				if(temp < min){
					min = temp
					g_dbclickPos = pos;
