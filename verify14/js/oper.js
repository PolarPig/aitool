$(function () {
	...
	// 保存编辑框内容
	$('.showqrs>.mask-left>.foot>span>.btn.confirm').on('click', () => {
		...
		let idxitem = idxdatas[g_check_qrsinfo.idx];
		g_modifAreaIdx = g_check_qrsinfo.idx
		let changeSub = idxitem.subidx
		...
		fetch(url, param).then(res => {
			...
			g_change_basic_idx = qrsChanged
			$('.mask.qrs').fadeOut();
			g_check_qrsinfo = undefined;
	...
	// 关闭编辑框
	$('.showqrs>.mask-left>.foot>span>.btn.cancel').on('click', () => {
		$('.mask.qrs').fadeOut();
	});
	// 打开数据
	...
	$('#upload').on('change', function () {
		...
		let fileName = /\.(ecg|ECG)$/.test(file.name);
		if (!fileName) {
			fileName = /\.(hecg|HECG)$/.test(file.name);
			if (!fileName) {
				alertx("请选择ecg文件");
				$('#m').fadeOut();
				return;
...
//			console.log(data);
		...
		try {
			let url = g_domain + '/dws/',
				param = {
					...
					// "functionId": "DWS001003015", 多张 过检报告预览(无参)
					// "functionId": "DWS001003016", 1张  过检报告预览(有参)
					// "functionId": "DWS001003017", 多张 过检报告预览(有参单波)
					body: data = JSON.stringify({
						"reqHead": {
							"functionId": "DWS001003017",
						},
						"body": {
							"anaecgFileId": anaecgFileid,
							"id": g_id,
			...
			let p = JSON.parse(param.body)
			// console.log(JSON.parse(param.body).reqHead.functionId);
			if (!g_paramFlag && g_situaFlag) {
				p.reqHead.functionId = "DWS001003015"
				param.body = JSON.stringify(p)
			} else if (g_paramFlag && !g_situaFlag) {
				p.reqHead.functionId = "DWS001003016"
				param.body = JSON.stringify(p)
			} else if (g_paramFlag && g_situaFlag) {
				p.reqHead.functionId = "DWS001003017"
				param.body = JSON.stringify(p)
			}
			fetch(url, param).then(res => {
				if (res.ok) {
				...
				if (name == ".icon_print") {
					...
					$(".r_content").hide()
					$(".c_content").hide()
					$(name).addClass("btns-group-active");
					$('.icon_preview span').text("返回分析");
					let html = ''
					if (Array.isArray(data.body.data) && data.body.data.length > 0) {
						for (let i = 0; i < data.body.data.length; i++) {
							html += `
								<div class='item'>
									<img src=${data.body.data[i]}>
								</div>
							`
						}
					} else {
						html += `
								<div class='item'>
									<img src=${data.body.data}>
								</div>
							`
					}
					$(".reportBox").show().append(html);
				}
	...
	$('.icon_preview,.show').on('click', function () {
		...
		if ($(this).hasClass('btns-group-active')) {
			...
			$(".item").remove(".reportBox");
			$(".reportBox").find("div").remove()
		} else {
			if (g_paramFlag) {
	...
	$('.icon_export').on('click', function () {
		...
		// "functionId": "DWS001003018", 多张 过检报告预览(无参)
		// "functionId": "DWS001003019", 1张  过检报告预览(有参)
		// "functionId": "DWS001003020", 多张 过检报告预览(有参单波)
		if (!g_paramFlag && g_situaFlag) {
			data.reqHead.functionId = "DWS001003018"
			// param.body = JSON.stringify(p)
		} else if (g_paramFlag && !g_situaFlag) {
			data.reqHead.functionId = "DWS001003019"
			// param.body = JSON.stringify(p)
		} else if (g_paramFlag && g_situaFlag) {
			data.reqHead.functionId = "DWS001003020"
			// param.body = JSON.stringify(p)
		}
